(* Functions in OCaml *)
(* Functions have value, which corresponds to a certain type, e.g int -> int 
...
(* Note that we cannot only annotate f when are parameters defined right after it.
   This does not compile *)
(* let (f: int -> int) x = x + 1  *)


(* Also note g is not equivalent to f *)
(* g x: int -> int means g is a function takes an argument x of any type, since not 
